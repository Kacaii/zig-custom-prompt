...
pub fn init(allocator: Allocator) ![]u8 {
    ...
    const zig_version_number = std.mem.trimRight(
        u8,
        zig_version_output,
        "\n",
    );

    const zig_section = try std.mem.concat(allocator, u8, &[_][]const u8{ set_color.yellow, "[ Zig", zig_version_number, "]", set_color.normal });
    return zig_section;
}
